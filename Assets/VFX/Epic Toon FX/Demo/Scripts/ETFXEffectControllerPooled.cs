...
namespace EpicToonFX
{
    public class ETFXEffectControllerPooled : MonoBehaviour
    {
        ...
        private void CleannowEffect()
        {
            StopAllCoroutines();

            if (nowEffect != null)
            {
                nowEffect.SetActive(false);
            }
        ...
        {
            //Pooling effect
            nowEffect = effectsPool[effectIndex];
            nowEffect.SetActive(true);

            if (disableLights && nowEffect.GetComponent<Light>())
            {
                nowEffect.GetComponent<Light>().enabled = false;
            }

            if (disableSound && nowEffect.GetComponent<AudioSource>())
            {
                nowEffect.GetComponent<AudioSource>().enabled = false;
            }
            ...
            ParticleSystem particleSystem = nowEffect.GetComponent<ParticleSystem>();
